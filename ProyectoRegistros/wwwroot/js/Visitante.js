...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (btnFinalizar) {
        btnFinalizar.addEventListener("click", async function (e) {
            ...
            try {
                const response = await fetch('/Visitante/Visitante/RegistroForm', {
                    method: 'POST',
                    body: formData
                });
    ...
    if (nombreInput) {
        nombreInput.addEventListener("blur", function () {
            ...
            fetch(`/Visitante/Visitante/BuscarAlumno?nombre=${encodeURIComponent(nombre)}`).then(r => r.json()).then(data => {
                if (data) {
